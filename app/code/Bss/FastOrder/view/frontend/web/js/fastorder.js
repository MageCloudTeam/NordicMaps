    ...
    $.widget('bss.fastorder', {
        ...
        _create: function () {
            ...
            $(document).on('click', '#bss-access-multiple', function () {
                localStorage.setItem('isAddingFromMultipleBox', true);
                if ($('#bss-multiple-sku').val() != "") {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            var responseData = response['childProduct'];
                                    ...
                                    $.ajax({
                                        ...
                                        success: function (response) {
                                            ...
                                            if (responseData.length > 0) {
                                                self.addRow(responseData.length);
                                                self.handleResponse(responseData, false);
                                            }
                                        },
                                    });
                            ...
                            var dataOtherProduct = response['otherProduct'];
                            if (dataOtherProduct != null) {
                                if (dataOtherProduct.length > 0) {
                                    self._getStackProductSelected(dataOtherProduct, opt.getProductSimple, 1);
                                }
                            }

                            if (response['otherProduct'] == "" || response['childProduct'] == "") {
                                localStorage.setItem('popupShowed', 0);
                                localStorage.setItem('isAddingNewGrouped', true);
                            }
                        },
                        error: function (er) {
                        }
            ...
            $(document).on('keyup','.input-text.qty',function(){
                ...
                else
                {
                    ...
                    $(this).closest('tr').find('.bss-fastorder-hidden.bss-addtocart-option').find('*').each(function(){
                        ...
                        if(name.startsWith('bss-fastorder-super_group['))
                        {
        ...
        closePopup: function () {
            var sortOrder = $('#bss-content-option-product').find('.bss-row-select').val();
            var editProductCache = window.editProductCache;
            var isEdit = editProductCache[sortOrder] !== undefined ? true : false;
            if (isEdit) {
                $(this.options.cancelButtonSelector).click();
            } else {
        ...
        _getProductSeller:function(){
            var self = this;
            if(self.options.getProductPopular != "")
            {
                var data = self.options.getProductPopular;
                var data3 = data['childProduct'];
                if(data3 != null)
                {
                    $.each(data3,function(key,element){
                        var skuChild = [];
                        ...
                        $.ajax({
                            url: self.options.urlChildSku,
                            data: {
                                parentProductId: key,
                                productList: skuChild
                            },
                            type: 'post',
                            dataType: 'json',
                            showLoader: true,
                ...
                if(data['otherProduct'].length >= 1)
                {
                    var dataOther = [];
                    var data2 = data['otherProduct'];
                    if (data2.length > 0) {
                        ...
                        self._getStackProductSelected(dataOther,self.options.getProductSimple,1);
                    }
                }
            }
        ...
        _removeKeySortOrder: function() {
            var allKeySortOrder = localStorage.getItem('allKeySortOrder');

            if (allKeySortOrder !== null) {
                var keys = allKeySortOrder.split('+');
            ...
            $('.bss-fastorder-row-qty .qty').each(function () {
                if ($(this).closest('.bss-fastorder-row-qty').find('.bss-product-id-calc').val() !== "") {
                    ...
                    if(productType != "grouped")
                    {
                        ...
                        totalPrice += current_qty * parseFloat(current_price);
                    }
                    else
                    {
        ...
        _getFormattedPrice: function (price,fomatPrice) {
            return priceUtils.formatPriceLocale(price, fomatPrice);
        },
        ...
        _reloadTotalPrice: function (el,fomatPrice) {
            ...
            if (productId != productCurId && product_tierprice['id_'+productId] !== undefined) {
        ...
        _getItemsLocalStorage: function (el, suggestCacheKey, sortOrder, dataMultiBox = null) {
            ...
            if ($('.bss-fastorder-multiple-form tbody tr:last').find('.bss-fastorder-autocomplete').is(':visible')) {
                $widget._scrollToTableBottom();
            }
        },
         ...
         * @private
         */
        _scrollToTableBottom: function() {
            $('.bss-fastorder-multiple-form tbody').animate({
                scrollTop: $('.bss-fastorder-multiple-form tbody')[0].scrollHeight
            }, 200);
        },
        ...
        _uploadCsv: function (el, csvUrl) {
            ...
            $widget.xhr = $.ajax({
                ...
                error: function () {
                    el.val('');
                    console.warn('Can not import csv');
                }
            });
        },
        ...
        addRow: function (rowNumber) {
            const lineCurrent = $('#bss-fastorder-form .bss-row').length;
            const lineUse = $('#bss-fastorder-form .bss-fastorder-row .bss-fastorder-autocomplete ul').length;
            const lineSurplus = lineCurrent - lineUse;
            if (rowNumber <= lineSurplus) {
                return;
            }
            const lineNew = rowNumber - lineSurplus;
            for (let i = 0; i < lineNew; i++) {
                $('#bss-fastorder-form .bss-addline').click();
            }
        },
        /* Validation Form*/
         ...
         * @private
         */
        _storagePopupContent: function(productData) {
            if (!_.isEmpty(productData) && !_.isEmpty(productData.product_id)) {
                var productId = productData.product_id;
                if (productData['popup_html'] !== undefined) {
                    if (window.popupContent === undefined) window.popupContent = {};
                    window.popupContent[productId] = {
                        productId: productId,
                        content: productData['popup_html'],
         ...
         * @private
         */
        _storageProductTierPrice: function(productData) {
            if (!_.isEmpty(productData) && !_.isEmpty(productData.product_id) && !_.isEmpty(productData['tier_price_' + productData.product_id])) {
                product_tierprice['id_' + productData.product_id] = productData['tier_price_' + productData.product_id];
            }
        },

        /**
         * @param res
         * @param isPrepopulatedQty
         */
        handleResponse: function (res, isPrepopulatedQty = true) {
            var displayPriceExclTax = "",
            ...
            if (res) {
                ...
                for (var key in res) {
                    ...
                    var html,
                        productData;
                    if (isPrepopulatedQty) {
                        productData = res[key][0];
                    } else {
                        productData = res[key];
                    }
                    if (res.hasOwnProperty(key)) {
                        $widget._storageProductTierPrice(productData);
                        $widget._storagePopupContent(productData);
                        ...
                        $('#bss-fastorder-form .bss-row').each(
                            function () {
                                ...
                                if (productData['child_product_id'] !== undefined) {
                                ...
                                if (productData['configurable_attributes'] !== undefined) {
