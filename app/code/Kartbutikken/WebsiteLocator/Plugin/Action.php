...
namespace Kartbutikken\WebsiteLocator\Plugin;


use Amasty\GeoipRedirect\Model\Source\Logic;
use Hryvinskyi\Base\Helper\VarDumper;
use Kartbutikken\WebsiteLocator\Model\PageResolverInterface;
...
class Action
{
    ...
    public function aroundDispatch(
        ...
        if ($this->scopeConfig->getValue('amgeoipredirect/restriction/first_visit_redirect')) {
            ...
            $isNotFirstTime = $session->getIsNotFirstTime() || ($request->getParam('___store') && $request->getParam('___from_store'));
        ...
        $resultRedirect->setHeader('Cache-Control', 'private, no-cache', true);
        if ($this->scopeConfig->getValue('amgeoipredirect/general/enable', $scopeStores, $currentStoreId)
            ...
            $country = getenv('HTTP_CF_IPCOUNTRY');
                ...
                foreach ($allStores as $store) {
                    ...
                    if ($country && $countries && strpos($countries, $country) !== false
                        ...
                        if ((strpos($currentUrl, $neededBaseUrl) === false)
                            ...
                            if ($this->sessionManager->getWillRedirect() !== false) {
                                $proceed($request);
                                $redirectStoreUrl = $this->pageResolver->get()->setTargetStore($store)->getRealUrl();
     ...
     * @return \Amasty\GeoipRedirect\Plugin\Action
     */
    ...
    protected function isHomePage()
    {
        ...
        if ($this->scopeConfig->getValue(StoreModel::XML_PATH_SECURE_IN_FRONTEND, ScopeInterface::SCOPE_STORE)) {
            $baseUrl = $this->scopeConfig->getValue(StoreModel::XML_PATH_SECURE_BASE_URL, ScopeInterface::SCOPE_STORE);
        } else {
            $baseUrl = $this->scopeConfig->getValue(StoreModel::XML_PATH_UNSECURE_BASE_URL, ScopeInterface::SCOPE_STORE);
        }
