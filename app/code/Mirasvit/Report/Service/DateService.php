...
class DateService implements DateServiceInterface
{
    ...
    public function getInterval($code, $inStoreTZ = false)
    {
        ...
        switch ($code) {
            ...
            case 'last_quarter':
                $month = intval($from->get(\Mirasvit\Core\Model\Date::MONTH) / 4) * 3 + 1;
            ...
            case self::THIS_YEAR:
                ...
                $to->setDay(1)
                    ->setMonth(1)
                    ->addDay($to->get(\Mirasvit\Core\Model\Date::LEAPYEAR) ? 365 : 364)
                    ->setTime('23:59:59');
            ...
            case self::THIS_WEEK:
                $weekday = $from->get(\Mirasvit\Core\Model\Date::WEEKDAY_DIGIT); #0-6
    ...
    public function getPreviousInterval($interval, $offset, $inStoreTZ = false)
    {
        ...
        $now = new \Mirasvit\Core\Model\Date(
     ...
     * {@inheritdoc}
     */
    public function toZendDate($date, $format = \Mirasvit\Core\Model\Date::ISO_8601)
    {
        if ($date instanceof \Mirasvit\Core\Model\Date) {
            return $date;
        }

        return new \Mirasvit\Core\Model\Date($date, $format);
    }
    ...
    /**
     * @param string|\Mirasvit\Core\Model\Date $from
     * @param string|\Mirasvit\Core\Model\Date $to
