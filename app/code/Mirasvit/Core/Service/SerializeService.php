...
class SerializeService
{
    ...
    public static function init()
    {
        if (self::isOldVersion()) {
            self::$serializer = \Laminas\Serializer\Serializer::factory('PhpSerialize');
            self::$jsoner     = \Laminas\Serializer\Serializer::factory('Json');
        } else {
    ...
    public static function decode($string)
    {
        ...
        try {
            new \ReflectionClass('Laminas\Json\Json');
        } catch (\Exception $e) {}

        // we use this because json_decode does not work correct for php5
        if (class_exists('Laminas\Json\Json', false)) {
            $useDecoder                                = \Laminas\Json\Json::$useBuiltinEncoderDecoder;
        ...
        if (class_exists('Laminas\Json\Json', false)) {
            \Laminas\Json\Json::$useBuiltinEncoderDecoder = $useDecoder;
        }
