...
class ReplaceImageTag
{
    ...
    private function convertImage($imageUrl, $htmlTag, $customSrcSetTag, $layout)
    {
        ...
        $mobileImage = '';
        if (preg_match_all("/data-src-mobile=(\"|\')([^<]+\.(png|jpg|jpeg))/mU", $htmlTag, $match, PREG_OFFSET_CAPTURE)) {
            $mobileImage = $match[2][0][0];
        }

        $webpUrl = $this->helper->convert($imageUrl);
        $webpMobileImage = $this->helper->convert($mobileImage);


        /**
         * Skip when extension can not convert the image
         */
        if ($webpUrl === $imageUrl && strpos($imageUrl, 'reference_images') === false) {
            return false;
        }
        ...
        if ($lazyload) {
            $pictureTag = $this->getPicture($layout)
                ...
                ->setMobileImage($webpMobileImage)
                ->setOriginalImagePath($this->helper->getImagePathFromUrl($imageUrl))
                ->setData('lazyload', $lazyload)
                ->toHtml();
        } else {
            $pictureTag = $this->getPicture($layout)
                ->setOriginalImage($imageUrl)
