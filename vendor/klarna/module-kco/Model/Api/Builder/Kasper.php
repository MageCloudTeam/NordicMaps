...
use Klarna\Kco\Model\Configuration\MandatoryFields;
use Klarna\Kco\Model\Configuration\Prefill;
use Klarna\Kco\Model\Configuration\ShippingOptions;
...
class Kasper implements BuilderInterface
{
    ...
    /**
     * @var \Klarna\Kco\Model\Configuration\Url
     */
    private \Klarna\Kco\Model\Configuration\Url $urlConfiguration;
    /**
     * @var FptValidator
     */
    ...
    public function __construct(
        ...
        Checkbox $checkboxConfiguration = null,
        ShippingOptions $shippingOptionsConfiguration = null,
        MandatoryFields $mandatoryFieldsConfiguration = null,
        \Klarna\Kco\Model\Configuration\Url $urlConfiguration = null,
        FptValidator $fptValidator = null,
        B2b $b2b = null,
        ...
        $this->checkoutConfiguration = $checkoutConfiguration ?: ObjectManager::getInstance()->get(
            Checkout::class
        );
        $this->prefillConfiguration = $prefillConfiguration ?: ObjectManager::getInstance()->get(
            Prefill::class
        );
        $this->checkboxConfiguration = $checkboxConfiguration ?: ObjectManager::getInstance()->get(
            Checkbox::class
        );
        $this->shippingOptionsConfiguration = $shippingOptionsConfiguration ?: ObjectManager::getInstance()->get(
            ShippingOptions::class
        );
        $this->mandatoryFieldsConfiguration = $mandatoryFieldsConfiguration ?: ObjectManager::getInstance()->get(
            MandatoryFields::class
        );
        $this->urlConfiguration = $urlConfiguration ?: ObjectManager::getInstance()->get(
            \Klarna\Kco\Model\Configuration\Url::class
        );
        $this->fptValidator = $fptValidator ?: ObjectManager::getInstance()->get(
            FptValidator::class
        );
        $this->b2b = $b2b ?: ObjectManager::getInstance()->get(
            B2b::class
        );
    ...
    private function generateRequest(CartInterface $quote)
    {
        ...
        $create = [
            ...
            'locale'            => $this->magentoToKlarnaLocaleMapper->getLocale()
        ];
         ...
         */
        if ($this->config->isShippingInIframe($store)) {
        ...
        foreach ($create['order_lines'] as $line) {
            $create['order_tax_amount'] += $line['total_tax_amount'] ?? 0;
        }
        if($create['order_tax_amount'] == 0) {
    ...
    {
        $options = $this->klarnaConfig->getExternalPaymentOptions($code);
        if ($options === null) {
            $options = [];
        }
        unset($options['label']);
    ...
    public function getOptions(CartInterface $quote)
    {
        ...
        $options = array_merge($options, [
            'allow_separate_shipping_address'
                => $this->shippingOptionsConfiguration->isSeparateShippingAddressAllowed($store),
            'phone_mandatory'
                => $this->klarnaConfig->isPhoneMandatorySupport($store),
        ...
        if ($this->klarnaConfig->isTitleMandatorySupport($store)) {
            $options['title_mandatory'] = $this->mandatoryFieldsConfiguration->isTitleMandatory($store);
        }

        if ($this->klarnaConfig->isPhoneMandatorySupport($store)) {
            $options['phone_mandatory'] = $this->mandatoryFieldsConfiguration->isPhoneMandatory($store);
        }

        $options['shipping_in_iframe'] = $this->config->isShippingInIframe($store);
    ...
    {
        if (!$this->klarnaConfig->isMerchantCheckboxSupport($store)) {
            return null;
        }
    ...
    private function handlePackstationSettings()
    {
        ...
        if ($this->config->isPackstationEnabled($store)
            && $this->config->isPackstationSupport($store)
        ) {
