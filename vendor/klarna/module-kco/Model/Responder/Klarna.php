...
class Klarna
{
    ...
    {
        $checkout = json_decode($request->getContent(), true);
        $this->logger->info('Klarna request body:');
        $this->logger->logApiRequest($checkout);

        return $this->dataObjectFactory->create(['data' => $checkout]);
    }
}
