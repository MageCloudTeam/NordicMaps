...
use Klarna\Kco\Model\Checkout\Configuration\ApiValidation;
...
class LayoutProcessorPlugin
{
    ...
    private function generateKlarnaIframe($quote)
    {
        ...
        try {
            $this->apiValidation->checkKpKcoEnabled($store);
            $this->apiValidation->checkKpApiEndpointUsed($store);
        ...
        try {
            ...
            $errorMessage = $this->determineErrorMessage($quote, $e);
    ...
    /**
     * Determines what error message to show via the Magento message manager when KCO init throws exception
     *
     * @param \Magento\Quote\Api\Data\CartInterface $quote
     * @param \Throwable                            $exception
     * @return string
     */
    private function determineErrorMessage($quote, $exception): string
    {
        $store = $quote->getStore();
        $kpEnabled = $this->config->getValue('payment/klarna_kp/active', ScopeInterface::SCOPE_STORES, $store);

        if ($kpEnabled) {
            return __('You cannot use both Klarna Checkout and Klarna Payments in the same website. ' .
            'Please disable one of them and try again.');
        }

        if ($this->usesKpApiVersion($store)) {
            return __('When using Klarna Checkout, please select a "Klarna Checkout" API version ' .
    ...
    /**
     * Let's us know if the current store's Klarna API version is a Klarna Payments version
     *
     * @param \Magento\Store\Model\Store $store
     * @return bool
     */
    private function usesKpApiVersion($store): bool
    {
        $klarnaApiVersion = $this->config->getValue('klarna/api/api_version', ScopeInterface::SCOPE_STORES, $store);
        // KP API versions are prefixed by "kp_". KCO versions have no prefix
        if ($klarnaApiVersion) {
            return strpos($klarnaApiVersion, 'kp_') === 0;
        }
        return false;
    }
}
